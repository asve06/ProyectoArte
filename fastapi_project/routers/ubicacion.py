...
@ubicacion.get("/ubicacion", response_model=list[UbicacionRead], tags=["ubicacion"])
def get_ubicacion(db: Session = Depends(get_db), skip: int = 0, limit: int = 10):
    return db.query(Ubicacion).offset(skip).limit(limit).all()

@ubicacion.post("/ubicacion", response_model=UbicacionRead, tags=["ubicacion"])
...
@ubicacion.get("/ubicacion/{ubicacion_id}", response_model=UbicacionRead, tags=["ubicacion"])
...
@ubicacion.delete("/ubicacion/{ubicacion_id}", status_code= status.HTTP_204_NO_CONTENT, tags=["ubicacion"])
...
@ubicacion.put("/ubicacion/{ubicacion_id}", response_model=UbicacionRead, tags=["ubicacion"])
